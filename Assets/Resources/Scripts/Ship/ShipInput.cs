...
namespace SpaceDenfece
{
    ...
    public class ShipInput : MonoBehaviour
    {
        ...
        private void Update()
        {
            roll = ScaleInputWithDeadzone(Input.GetAxis("Horizontal"), GameSettings.deadzone);
        ...
        private float ScaleInputWithDeadzone(float input, float deadzone)
        {
            float num = Mathf.InverseLerp(deadzone, 1f, Mathf.Abs(input));
            float value = num * ((!(input >= 0f)) ? (-1f) : 1f);
